...
export interface AlpacaPaperAdapterConfig {
  ...
  baseUrl: string;
  /**
   * Position quantity drift tolerance (shares, fractional allowed) before reconciliation fails.
...
export class AlpacaPaperAdapter implements BrokerAdapter {
   ...
   *   sellPercent      → { qty: String(floorFractionalShares(posQty × fraction)) }
  ...
  toAlpacaOrder(
    ...
    if (intent.side === "buy") {
      ...
      switch (size.type) {
        ...
        case "sellPercent": {
          const fraction = Math.max(0, Math.min(1, size.fraction));
          return { ...base, qty: String(floorFractionalShares(posQty * fraction)) };
        }
        case "closePosition":
